...
function M.setup(user_opts)
  ...
  function! CommentOperator(type) abort
    let reg_save = @@
    execute "lua require('vendor/nvim-comment').operator('" . a:type. "')"
    let @@ = reg_save
  endfunction
  ]]

  vim.api.nvim_call_function("execute", { vim_func })
  vim.api.nvim_command("command! -range CommentToggle lua require('vendor/nvim-comment').comment_toggle(<line1>, <line2>)")
  ...
  if M.config.create_mappings then
    ...
    api.nvim_set_keymap(
      ...
      "<Cmd>lua require('vendor/nvim-comment').select_comment_chunk()<CR>",
      opts
    )
    api.nvim_set_keymap(
      "o",
